<template>
    <div class="ibox">
       <p class="title">
           <span class="left">共{{listData['total']}}个设计作品</span>
           <span class="right redact" @click='isShowDel()'>{{isRedact}}</span>
         
       </p>
       <div class="designbox">
           <div class="designItem" v-for="(item,key,index) in listData['data']" :key="key">
               <div class="imgbox" @click="openUnit(item)">
                     <!-- {{item}} -->
                   <span class="del" v-if="isDel" @click.stop="removeItem(item,index)"></span>
                   <!-- <span class="copy" v-if='isDel' @click.stop="CopyItem(item)"></span> -->
                   <span class="mPai" v-if="item.resource_type == 1">整装设计</span>
                   <span class="mPais" v-if="item.resource_type == 2">空间设计</span>
                   <img :src="R_IMG +item.complete_img" alt="" class="w100">
               </div>
               <p class="address">{{item.complete_title}}</p>
               <p class="time">{{dataTime(item.add_time)}}</p>
           </div>
           
       </div>
    </div>
</template>
<script>
import {bus} from "@/bus.js";
export default {
    data(){
        return{
            isDel:false,
            isAjax:false,
            isRedact:"编辑",
            R_IMG:R_IMG,
            page_num: 1,
            user_id:'',//this.gValue.checkedArray.init.userId
            listData:{data:[],total:''},
            paramList:{
                "edition_num": "8",
                // "page_num": 1,
                "per_mdb_id": "3",
                "row_num": "10",
                "user_id":'',
                "type": 2
            },
            serverName:"TD_HSV1_3Ddesign_solr_v3"
        }
    },
    methods:{
        isShowDel(){
            this.isDel = !this.isDel;
            this.isRedact = this.isDel?'完成':'编辑';
        },
        ajax_post_list(){//列表
            this.gValue.checkedArray.init.userId && (this.user_id = this.gValue.checkedArray.init.userId);
            if(this.user_id){ 
                    this.paramList.page_num = this.page_num;
                    this.isAjax = true;
                    this.paramList["city_id"] = this.gValue.checkedArray.init.city_id;
                    this.paramList["g_city_name"] =this.gValue.checkedArray.init.cityName;
                    this.paramList["user_id"] = this.user_id;
                    ajax_post({"serviceName":"TD_HSV1_3Ddesign_solr_v3","param":this.paramList,"format":"json","need":1},(res)=>{
                        if(JSON.parse(res).rows[0]){
                            res = JSON.parse(res).rows[0];
                            this.listData.total = res.total_num;
                            var dataList = JSON.parse(res.resource_info);
                                if(dataList.length > 0){
                                    this.listData.data = this.listData.data.concat(dataList);
                                    this.isAjax = false
                                } 
                        }
                })
            }    
        },
         dataTime(now) {
            now = new Date(now * 1000);
            var year = now.getFullYear();
            var month = now.getMonth() + 1;
            var date = now.getDate();
            var hour = now.getHours();
            var minute = now.getMinutes();
            var second = ("0" + now.getSeconds()).slice(-2);
            return year + "-" + month + "-" + date + "   " + hour + ":" + minute + ":" + second;
        },
        removeItem(data,index) {
            var cof = confirm("确认删除");
            if (cof) {
                var jsondata = {
                    complete_id: data.complete_id,
                    user_id: this.user_id,
                    is_delete: 1
                };
                ajax_post({
                    "format": "json",
                    "needAll": "1",
                    "param": jsondata,
                    "serviceName": "TD_HSV1_CompleteResource_De"
                },(res)=>{
                    var index = this.listData.data.indexOf(data);
                    this.listData.data.splice(index,1);
                   
                });
            }
        },
        CopyItem(data){
            ajax_post({
                "format": "json",
                "needAll": "1",
                "param": {
                    "complete_id": item.complete_id,
                    "user_id": this.user_id
                },
                "serviceName": "TD_BUV1_CompleteResource_info_F"
            },function (res) {
                if (getCookieValue('isApp')) {
                    native_invoke('copymydesigner', { designinfo: JSON.stringify(res)});
                };
                this.ajax_post_list();
            });

        },
        openUnit(item) {
            ajax_post({
                "format": "json",
                "needAll": "1",
                "param": {
                    "complete_id":item.complete_id,
                    "user_id":this.user_id
                },
                "serviceName": "TD_BUV1_CompleteResource_info_F"
            }, function (res) {
                if (getCookieValue('isApp')) {
                    native_invoke('mydesign', { designinfo: res });
                }
            });
        },
         busType(data){
            if(data == '2'){
                if(!this.isAjax){
                    this.paramList.page_num=this.page_num++;
                    this.ajax_post_list();
                }
            }
        },
        busFn(data){//获取点击筛选项
            this.listData.data = [];
            this.page_num = 1;
            if(JSON.stringify(this.gValue.checkedArray.data) != "{}"){
                var selectedArray = [];
                for (var i in this.gValue.checkedArray.data){
                  var str = i + ":" + this.gValue.checkedArray.data[i];
                  selectedArray.push(str);
                }
                this.paramList['selected_list'] = selectedArray.join(',');
                this.ajax_post_list();
            }else{
                delete this.paramList['selected_list'];
                this.ajax_post_list();
            }    
        }
      
    },
    created(){
        this.ajax_post_list();//c初始化数据
        bus.$on('type',this.busType);
        bus.$on('isajax',this.busFn);
        bus.$on('userInit', this.ajax_post_list);
    },
    destroyed(){
        bus.$off("isajax",this.busFn);
        bus.$off("type",this.busType);
        bus.$off('userInit', this.ajax_post_list);
    }
}
</script>
<style lang="less" scoped>
    .ibox{
        background-color: #fff;
        .title{
            width:100%;
            height:1.18rem;
            font-size: 0.33rem;
            line-height: 1.18rem;
            padding:0 0.3rem; 
            box-sizing: border-box;
            .redact{
                font-size: 0.36rem;
                color:#ff8800;
            }
        }
        .designbox{
            width:100%;
            padding:0 0.3rem;
            box-sizing: border-box;
            
            overflow: hidden;;
            .designItem{
                &:nth-child(2n){
                    margin-left:0.3rem;
                }
                width:4.95rem;
                height:5.44rem;
                float: left;
                .imgbox{
                    width: 100%;
                    height:3.7rem;
                    position: relative;
                    margin-bottom: 0.36rem;
                    font-size: 0.12rem;
                    .mPai{
                        width:1.58rem;
                        height:0.5rem;
                        position: absolute;
                        left:0;top:0;
                        text-align: center;
                        line-height: 0.5rem;
                        font-size: 0.3rem;
                        color:#fff;
                        background-color: #ff8800;
                    }
                    .mPais{
                        width:1.58rem;
                        height:0.5rem;
                        position: absolute;
                        left:0;top:0;
                        text-align: center;
                        line-height: 0.5rem;
                        font-size: 0.3rem;
                        color:#ff8800;
                        background-color: #fff;
                    }
                    .del{
                        width:0.74rem;
                        height:0.74rem;
                        border-radius: 50%;
                        background-color: rgba(0,0,0,0.6);
                        position: absolute;
                        top:0.24rem;right:0.24rem;
                        background-image: url('//static.youjiagou.com/youjiagou/resources/images/3dsjq/3dsjandroid/del.png');
                        background-repeat: no-repeat;
                        background-size: cover;
                        
                    }
                    .copy{
                        width:0.74rem;
                        height:0.74rem;
                        border-radius: 50%;
                        background-color: rgba(0,0,0,0.6);
                        position: absolute;
                        top:0.24rem;right:1.22rem;
                        background-image: url('//static.youjiagou.com/youjiagou/resources/images/3dsjq/3dsjandroid/copy.png');
                        background-repeat: no-repeat;
                        background-size: cover;
                    }
                }
                .address{
                    font-size: 0.36rem;
                    color:#333;
                    margin-bottom: 0.25rem;
                    width:100%;
                    overflow:hidden;
                    text-overflow: ellipsis;
                    white-space: nowrap;
                }
                .time{
                    color:#999;
                    line-height: 1;
                    font-size: 0.25rem;

                }
            }
        }

    }
</style>
